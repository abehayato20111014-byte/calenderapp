...
function deleteEventFromModal() {
    ...
    const evtIndex = state.events.findIndex(e => e.id === id);
    if (evtIndex === -1) return;

    const targetEvt = state.events[evtIndex];

    if (!confirm(`予定「${targetEvt.title || '無題'}」を削除しますか？`)) {
        return;
    }

    if (typeof recordStateForUndo === 'function') {
...
function listenToGroupEvents(groupId) {
    ...
    ref.on('value', snapshot => {
        ...
        Object.values(cloudEvents).forEach(cloudEvt => {
            ...
            if (idx >= 0) {
                state.events[idx] = { ...state.events[idx], ...cloudEvt };
            } else {
                state.events.push(cloudEvt);
            }
        });
...
function openEventModal(evt = null) {
    ...
    const allDayCheckbox = document.getElementById('eventAllDayInput');
    const repeatCheckbox = document.getElementById('eventRepeatInput');
    const lockedCheckbox = document.getElementById('eventLockedInput');
    const startInput = document.getElementById('eventStartInput');
    const endInput = document.getElementById('eventEndInput');
...
function openShareGroupModal() {
    ...
    if (container) {
        ...
        if (groupKeys.length === 0) {
            container.innerHTML = '<div style="font-size:0.8rem; color:#94a3b8; text-align:center; padding:12px;">参加中のグループがありません</div>';
        } else {
            const currentShared = state.currentEditingEvent ? getSharedGroupIds(state.currentEditingEvent) : [];
            groupKeys.forEach(gId => {
                const grp = state.groups[gId];
                const isChecked = currentShared.includes(gId);
                const label = document.createElement('label');
                label.style.cssText = 'display:flex; align-items:center; gap:8px; padding:6px; font-size:0.85rem; cursor:pointer;';
                label.innerHTML = `
                    <input type="checkbox" value="${gId}" ${isChecked ? 'checked' : ''} class="share-group-checkbox">
                    <span>👨‍👩‍👧‍👦 ${grp.name}</span>
                `;
                container.appendChild(label);
            });
...
// モーダル内で共有を確定する処理
function confirmShareEvent() {
    if (!state.currentEditingEvent) {
        closeShareGroupModal();
        return;
    }

    const checkboxes = document.querySelectorAll('.share-group-checkbox:checked');
    const selectedGroupIds = Array.from(checkboxes).map(cb => cb.value);

    state.currentEditingEvent.sharedGroupIds = selectedGroupIds;
    state.currentEditingEvent.ownerId = state.currentEditingEvent.ownerId || state.profile.userId;
    state.currentEditingEvent.ownerName = state.currentEditingEvent.ownerName || state.profile.userName;

    if (selectedGroupIds.length > 0 && typeof syncSharedEventToCloud === 'function') {
        syncSharedEventToCloud(state.currentEditingEvent);
    }

    saveData();
    closeShareGroupModal();
    renderAllViews();
    showToast('グループ共有設定を更新しました');
}

// 途切れていた initModalEvents 関数の補完
function initModalEvents() {
    const addEventBtn = document.getElementById('addEventBtn');
    if (addEventBtn) addEventBtn.addEventListener('click', () => openEventModal());
    ...
    if (allDayCheckbox) {
        allDayCheckbox.addEventListener('change', (e) => {
            if (typeof toggleTimeInputsForMode === 'function') {
                toggleTimeInputsForMode(e.target.checked);
            }
        });
    }
    ...
    if (addNotifBtn) {
        addNotifBtn.addEventListener('click', () => {
            if (typeof addNotificationToModalList === 'function') {
                addNotificationToModalList();
            }
        });
    }
    ...
    if (saveFavBtn) {
        saveFavBtn.addEventListener('click', () => {
            if (typeof saveCurrentAsFavorite === 'function') {
    ...
    if (saveEventBtn) {
        saveEventBtn.addEventListener('click', () => {
            if (typeof saveEventFromModal === 'function') {
                saveEventFromModal();
            }
        });
    }
